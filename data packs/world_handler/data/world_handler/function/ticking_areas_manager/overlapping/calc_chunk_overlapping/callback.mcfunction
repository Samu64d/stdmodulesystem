...
execute if score ::world_handler::ticking_areas_manager temp0 = ::const TRUE run return 0

# Check if the chunk is inside the ticking area including border
scoreboard players operation ::in include_border = ::const TRUE
execute if function world_handler:ticking_areas_manager/ticking_area_descriptor/is_chunk_inside run scoreboard players add ::out border_overlapping_index 1
