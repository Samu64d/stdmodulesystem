...
#
# Calculate the overlapping index and the border overlapping index of a chunk inside a ticking area
#> Scores
#  @overlapping_index (out): the overlapping index
#  @border_overlapping_index (out): the border overlapping index
#> Storage
...
scoreboard players set ::out border_overlapping_index 0

# Check for overlapping with the same area
scoreboard players operation ::in include_border = ::const FALSE
execute store result score ::world_handler::ticking_areas_manager temp0 run function world_handler:ticking_areas_manager/ticking_area_descriptor/is_chunk_inside
execute if score ::world_handler::ticking_areas_manager temp0 = ::const TRUE run scoreboard players add ::out overlapping_index 1
execute if score ::world_handler::ticking_areas_manager temp0 = ::const FALSE run scoreboard players add ::out border_overlapping_index 1
...
data modify storage io: list_ref set value "io: ticking_area_uuid_list"
