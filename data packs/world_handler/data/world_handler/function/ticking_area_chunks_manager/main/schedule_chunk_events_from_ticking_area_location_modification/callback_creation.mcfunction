...
# Check if the chunk is inside the old ticking area
...
execute if score ::world_handler::ticking_area_chunks_manager temp1 = ::const FALSE if score ::out overlapping_index matches 1 run function world_handler:ticking_area_chunks_manager/main/common/schedule_chunk_for_in_area
