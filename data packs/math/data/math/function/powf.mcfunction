...
# math:powf
#
# Calculate the n-th power value of a number with arbitrary fixed point precision
#> Scores
#  @value (in): the number
#  @n (in): the power
...
execute store result score ::math temp0 run function math:is_evenf
...
# Calculate pow(x, [n])
scoreboard players operation ::in value = ::in n
execute store result score ::math n_int run function math:get_integer_partf
scoreboard players operation ::math pow_int = ::math mag_value
execute if score ::math n_int matches 1.. run function math:powf/handle_integer_part

# Calculate pow(x, n - [n])
scoreboard players operation ::in value = ::in n
execute store result score ::math n_fract run function math:get_fractional_partf
scoreboard players operation ::math pow_fract = ::math mag_value
scoreboard players operation ::math pow_fract *= ::int 10000
execute if score ::math n_fract matches 1.. run function math:powf/handle_fractional_part
...
execute if score ::math inverse = ::const TRUE store result score ::math pow run function math:invf
