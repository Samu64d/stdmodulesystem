...
# math:acoshf
#
# Get the hyperbolic arccosine value of a given ratio with arbitrary fixed point precision
#> Scores
...
scoreboard players set ::in size 2
execute store result score ::in max run function math:fixed_point/calc_const_value
scoreboard players operation ::in value = ::in t
execute store result score ::math is_in_range run function math:is_in_range

# In range
execute if score ::math is_in_range = ::const TRUE run function math:acoshf/in_range

# Out range
execute if score ::math is_in_range = ::const FALSE run function math:acoshf/out_range
